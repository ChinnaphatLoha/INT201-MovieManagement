...
describe('Movie Class', () => {
  ...
  test('Delete movie by title', () => {
    const output = movieDatabase.deleteMovieByTitle('inception');
    ...
    expect(output).toBe(
      `a movie object {"title":"Inception","director":"Christopher Nolan","year":2010,"genre":"Science Fiction"} is deleted`
    );
  });
  test('Delete movie by title with case insensitive', () => {
    const output = movieDatabase.deleteMovieByTitle('THE DARK KNIGHT');
    expect(movieDatabase.movies).toHaveLength(2);
    expect(movieDatabase.movies).toEqual([
      {
        title: 'Inception',
    ...
    expect(output).toBe(
      `a movie object {"title":"The Dark Knight","director":"Christopher Nolan","year":2008,"genre":"Action"} is deleted`
    );
  });
  test('Delete movie by title (movie not found)', () => {
    const output = movieDatabase.deleteMovieByTitle('Avatar');
    expect(movieDatabase.movies).toHaveLength(3);
    expect(movieDatabase.movies).toEqual([
      {
        title: 'Inception',
    ...
    expect(output).toBe('no movie deleted');
  });
});
